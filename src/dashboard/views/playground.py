...
from src.dashboard.utils.data_manager import get_player_stats
from src.dashboard.utils.model_manager import load_model, get_model_features
from src.dashboard.utils.plotting import plot_gauge
...
def render_playground(df, available_models):
    """Render the Playground tab."""
    st.header('🎮 Match Prediction Playground')
    ...
    with col1:
        ...
        p1 = st.selectbox('Select Player 1', all_players, index=all_players.index('Nadal R.') if 'Nadal R.' in all_players else 0, key='p1')
        ...
        if is_v2 and p1 in player_state:
            ...
            # Surface specific logic later, default to global for now or specific if surface selected
    ...
    with col2:
        ...
        p2 = st.selectbox('Select Player 2', all_players, index=all_players.index('Federer R.') if 'Federer R.' in all_players else 1, key='p2')
